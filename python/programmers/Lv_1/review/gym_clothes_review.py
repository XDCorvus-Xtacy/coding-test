...
수업을 들을 수 없기 때문에 체육복을 적절히 빌려 최대한 많은 학생이 체육수업을 
들어야 합니다.
...
def solution(n, lost, reserve):
    ...
    # reserve이면서 lost인 학생 제거
    lost_self = list(set(lost) & set(reserve))
    if lost_self:
        for i in lost_self:
            sort_lost.remove(i)
            sort_reserve.remove(i)

    # reserve에서 앞번호부터 줄 수 있으면 한 명씩 나눠주기
