...
각 유저별로 처리 결과 메일을 받은 횟수를 배열에 담아 return 하도록 solution 
함수를 완성해주세요.
제한사항
2 ≤ id_list의 길이 ≤ 1,000
1 ≤ id_list의 원소 길이 ≤ 10
id_list의 원소는 이용자의 id를 나타내는 문자열이며 알파벳 소문자로만 이루어져 
있습니다.
...
1 ≤ k ≤ 200, k는 자연수입니다.
return 하는 배열은 id_list에 담긴 id 순서대로 각 유저가 받은 결과 메일 수를 
담으면 됩니다.
'''
def solution(id_list, report, k):
    ...
    for string in report:
        ...
        report_cnt[reported].add(reporter)
        
    for reported, reporter in report_cnt.items():
        if len(reporter) >= k:
            for i in reporter:
                mail_cnt[i] += 1
        
    for key in id_list:
        answer.append(mail_cnt[key])
    return answer
